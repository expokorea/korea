...
private:
    ...
    struct Halfedge *ELleftbnd(struct VorPoint *p);
    ...
    int right_of(struct Halfedge *el,struct VorPoint *p);
    
    struct Site *rightreg(struct Halfedge *he);
    struct Edge *bisect(struct	Site *s1,struct	Site *s2);
    float dist(struct Site *s,struct Site *t);
    struct Site *intersect(struct Halfedge *el1, struct Halfedge *el2, struct VorPoint *p=0);
